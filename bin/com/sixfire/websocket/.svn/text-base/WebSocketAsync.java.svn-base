...
public class WebSocketAsync extends Thread {
	...
	public void send(byte[] message, int offset, int len)
			throws java.io.IOException {
		if (!mHandshakeComplete) {
			throw new IllegalStateException("Handshake not complete");
		}
		...
		switch (l_bytes) {
		case 3:
			blen_buf[idx] = ((byte) (0x80 | (len >> 14)));
			idx++;
		case 2:
			blen_buf[idx] = ((byte) (0x80 | (0x7f & (len >> 7))));
			idx++;
		case 1:
	...
	class ReadState {
		...
		void saveBinaryData() {
			if (binaryData == null) {
				binaryData = Arrays.copyOfRange(buf, curIndex, curIndex
						+ (binaryLen - binaryRead));
				dbg("Copying buffer from: " + curIndex + ", length: "
						+ (binaryLen - binaryRead)
						+ ". Results in an array of length: "
						+ binaryData.length);
			} else {
				System.arraycopy(buf, curIndex, binaryData, binaryRead,
						(binaryLen - binaryRead));
				dbg("Appending buffer from: " + curIndex + ", length: "
						+ (binaryLen - binaryRead)
						+ ". Results in an array of length: "
						+ binaryData.length);
			}
		...
		void savePartialBinaryData() {
			...
			dbg("Saving partial binary data - " + (read - curIndex)
					+ " - all binary length: " + binaryLen
					+ " - cur binary read: " + binaryRead);
		...
		String state() {
			String o = "";
			switch (state) {
			case MATCH_START_BINARY_LGT:
				o = "Match start binary len";
				break;
			case MATCH_START_MSG:
				o = "Match start message";
				break;
			case READING_BINARY_DATA:
				o = "Read binary data";
				break;
			case READING_BINARY_LGT:
				o = "Read binary len";
				break;
			case READING_MSG:
				o = "Read message";
	...
	private boolean readBinary(ReadState rs) {
		if (rs.hasAtLeastMore(rs.binaryLen - rs.binaryRead)) {
			dbg("Buffer has " + (rs.binaryLen - rs.binaryRead)
					+ " bytes, finish buffering binary-data frame");
			rs.saveBinaryData();
			try {
